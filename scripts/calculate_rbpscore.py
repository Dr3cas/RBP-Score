...
DEFAULT_CONFIG = {
    ...
    "normalization": {"evalue_max_log": 150.0},
}
...
def evalue_to_score(evalue, max_log_evalue=150.0):
    """
    Convert BLAST e-value into a normalized score in [0,1].

    E-values are inverse and logarithmic: lower e-values indicate stronger
    evidence. The function first converts the e-value with -log10(evalue),
    then applies compressed logarithmic scaling.

    This avoids early saturation: very strong hits such as 1e-50 already get
    high support, while stronger hits such as 1e-100 or 1e-150 can still be
    distinguished.
    """
    e = safe_float(evalue, None)
    ...
    neg_log = -math.log10(e)

    if neg_log <= 0:
        return 0.0

    score = math.log1p(neg_log) / math.log1p(float(max_log_evalue))
...
def main():
    ...
    normalization_cfg = cfg.get("normalization", {"evalue_max_log": 150.0})
    ...
    for q in query_ids:
        ...
        for s in db_ids:
            ...
            if hit:
                ...
                ev_score = evalue_to_score(
                    hit["evalue"],
                    normalization_cfg["evalue_max_log"],
                )
